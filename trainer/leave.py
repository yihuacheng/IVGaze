...
def main(config):
    ...
    print("===> Read data <===")
    data.origin, folder = ctools.readfolder(
                        data.origin, 
                        [config.person], 
                        reverse=True
                    )

    data.norm, folder = ctools.readfolder(
                        data.norm, 
                        [config.person], 
                        reverse=True
                    )
    ...
    with open(os.path.join(savepath, "train_log"), 'w') as outfile:
        ...
        for epoch in range(1, params.epoch+1):
            for i, (data, anno) in enumerate(dataset):
                ...
                for key in anno.keys(): anno[key] = anno[key].cuda()
...
if __name__ == "__main__":
    ...
    config = edict(yaml.load(open(sys.argv[1]), Loader=yaml.FullLoader))
